...
string left_shift_1(string s){
	string ans=s;
	char first=s[0];
	for(int i=0;i<s.length()-1;i++){
		ans[i]=s[i+1];
	}
	ans[s.length()-1]=first;
	return ans;
}
string left_shift_2(string s){
	string ans=s;
	char first=s[0];
	char second=s[1];
	...
	ans[s.length()-2]=first;
	ans[s.length()-1]=second;
	return ans;
}
